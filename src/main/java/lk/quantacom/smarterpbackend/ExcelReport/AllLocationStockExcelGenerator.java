...
public class AllLocationStockExcelGenerator {
    private static final DecimalFormat df = new DecimalFormat("#,##0.00");

    private List<stockLocationResponse> responseList1;

    private List<LocationStockDetailsResponse> responseList;
    ...
    public AllLocationStockExcelGenerator(List<stockLocationResponse> responseList1, List<LocationStockDetailsResponse> responseList,List<Double> doubleList,Double aDouble ) {
    ...
    private void writeHeader() {
        ...
        sheet = workbook.createSheet(" LOCATION STOCK REPORT ");
        ...
        createCell(row, 5, "COLOR", style);
        int columnCount = 6;
        for (stockLocationResponse response : responseList1) {
            createCell(row, columnCount++, response.getBRANCH_NAME(), style);
        }
    ...
    private void writeHeaderTopic() {
        ...
        createCell(row2, 0, " LOCATION STOCK REPORT ", style2);
...
//        createCell(row5, 3, "", style3);
    ...
    private void write() {
        ...
        for (LocationStockDetailsResponse record : responseList) {
            Row row = sheet.createRow(rowCount++);
            int columnCount = 0;
            createCell(row, columnCount++, record.getItem_code(), style);
            ...
            createCell(row, columnCount++, record.getColor(), style);

            for (StockLocationListResponse listResponse : record.getResponseList()) {
                int columnCount1 = 6;
                for (stockLocationResponse response : responseList1) {
                    if (listResponse.getBranchid().longValue() == response.getBRANCH_ID()) {
                        createCell(row, columnCount1++, listResponse.getSoresQty(), style);
                    }
                    columnCount1++;
                }
                createCell(row, (columnCount1++)-1, record.getTotStores_qty(), style1);
            }
        }
        ...
        for (Double aDouble:doubleList){
            createCell(row, columnCount2++, aDouble, style1);
        }
        createCell(row, columnCount2++, aDouble, style4);
    }

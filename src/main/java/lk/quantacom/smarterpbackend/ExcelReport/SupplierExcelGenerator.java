...
public class SupplierExcelGenerator {
    private static final DecimalFormat df = new DecimalFormat("#,##0.00");

    private List<getExcelCreditorOutstandingResponse> responseList;

    private XSSFWorkbook workbook;

    private XSSFSheet sheet;

    public SupplierExcelGenerator(List<getExcelCreditorOutstandingResponse> responseList) {
    ...
    private void writeHeader() {
        ...
        createCell(row, 0, "GRN NO", style);
        createCell(row, 1, "GRN DATE", style);
        createCell(row, 2, "SUPPLIER IN DATE", style);
        createCell(row, 3, "SUP IN NO", style);
        createCell(row, 4, "BRANCH ID", style);
        createCell(row, 5, "NET AMOUNT", style);
        createCell(row, 6, "DUE AMOUNT", style);
    ...
    private void writeHeaderTopic() {
        ...
        createCell(row2, 0, "Supplier Debit Report", style2);
    ...
    private void write() {
        ...
        for (getExcelCreditorOutstandingResponse record : responseList) {
            if (record.list != null) {
                ...
                int columnCount1 = 0;
                createCell(row1, columnCount1++, record.getSupCode()+" - "+record.getSupName(), style1);

                for (ExcelSupplierOutstandingResponse response : record.getList()) {
                    Row row = sheet.createRow(rowCount++);
                    int columnCount = 0;
                    createCell(row, columnCount++, response.getGrn_no(), style);
                    createCell(row, columnCount++, response.getGrn_date(), style);
                    createCell(row, columnCount++, response.getSup_in_date(), style);
                    createCell(row, columnCount++, response.getSup_in_no(), style);
                    createCell(row, columnCount++, response.getBranch_id(), style);
                    createCell(row, columnCount++, df.format(response.getNet_amount()), style3);
                    createCell(row, columnCount++, df.format(response.getDue_amount()), style3);
                }
            }
        }
    }
